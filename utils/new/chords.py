    ...
    with open(gt_path, 'r') as f:
        for gt_chord_line in f:
            ...
            if not ch_num:
                ...
                ch_name = nums_to_chord(ch_num)
            test.append(str(start_time) + '	' + str(end_time) + '	' + ch_name)
    return test
...
def preds_to_lab(y, hop_size, fs, category, save_path, song_name):
    ...
    for i, chord_name in enumerate(chord_names):
        if chord_name == y_prev:
            continue
        end_time = i * tw
        results.append(str(start_time) + '	' + str(end_time) + '	' + chord_name)
        start_time = end_time
...
def data_bound(t, tbdrys, boundtype):
    if t <= tbdrys[0]:
        return 1  # mb 0
    if t >= tbdrys[-1]:
        return len(tbdrys)
...
def create_chords_list(category):
    ...
    for note in get_note_names():
        for chord_type in chord_types:
            num_to_ind[note2num(note)+ ":" + chord_type[0]] = len(ind_to_name)
            ind_to_name.append(note + ":" + chord_type[0])
...
def chord_to_nums(chord, category):
    ...
    if get_types_and_inv(category)[type_name]:
        bass = '/' + bass if bass else bass
    else:
        bass = ''
    return str(root) + ":" + type_name + bass
...
def create_inversions(type_name, comps):
    inversions = {}
    for comp in comps:
        inversions[type_name + '/' + comp] = comps
    return inversions
...
def nums_to_chord(nums):
    ...
    return get_note_names()[int(root)] + ':' + type_name


if __name__ == '__main__':
    path = 'C:/Users/Daniil/Documents/Git/vkr/data/gt/TheBeatles/01_-_Please_Please_Me/01_-_I_Saw_Her_Standing_There.lab'
    # np.savetxt("C:/Users/Daniil/Documents/Git/baseline/converted.csv", t_converter(path,"MirexRoot"), delimiter=",", fmt='%s')
    # np.savetxt("C:/Users/Daniil/Documents/Git/baseline/converted.lab", t_converter(path, "MirexMajMin"), delimiter=",", fmt='%s')
    # np.savetxt("C:/Users/Daniil/Documents/Git/baseline/converted.lab", t_converter(path, "MirexMajMinBass"), delimiter=",", fmt='%s')
    # np.savetxt("C:/Users/Daniil/Documents/Git/baseline/converted.lab", t_converter(path, "MirexSevenths"), delimiter=",", fmt='%s')
    #np.savetxt("C:/Users/Daniil/Documents/Git/baseline/converted.lab", t_converter(path, "MirexRoot"), delimiter=",",
              # fmt='%s')
    # convert_gt('C:/Users/Daniil/Documents/Git/vkr/data/gt/TheBeatles/01_-_Please_Please_Me/01_-_I_Saw_Her_Standing_There.lab', hop_size = 512, fs = 11025, song_length=1939104, category ='MirexMajMin')
    a, b = create_chords_list('MirexRoot')
    print(a)